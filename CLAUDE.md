...
**Tauri commands (`crates/athenaeum-tauri/src/commands/`)** — 233 functions across 22 modules (re-measured 2026-08-31; was 232/23 on 2026-08-24 — the calibrated-export-v2 cycle deleted the `lights` module (4 commands: `get_light_calibration_readiness`/`get_light_calibration_details`/`start_light_calibration`/`cancel_light_calibration`) wholesale, and other tasks in the same cycle net-added 5 elsewhere. `cache` is an empty placeholder module post-T6 — still declared in `mod.rs` so it counts as a module, contributes 0 commands). Each has a sibling in `crates/athenaeum-web/src/routes/` with the same name and surface:
...
**Gate — masters-built strictness** (`api::lights::check_mode_ready` + `compute_export_readiness`, the ONE gate for export AND send): every calibration set linked anywhere in the frame set's tree must already be a built master (`ExportReadiness.raw_sets_without_master` / `raw_set_ids_without_master` blocks with "Build masters first — N sets without a master" + a `→ Coverage` deep-link); a light with **zero** calibration links blocks with "N lights have no calibration links" (`unlinked_lights`); a partially-linked light (e.g. dark only) does NOT block — it calibrates best-effort with an honest `CALSTAT`. No auto-building of masters — the blocker routes the user to Coverage. `ExportReadiness` is mode-less (`{ total, unlinkedLights, rawSetsWithoutMaster, rawSetIdsWithoutMaster, fileCounts }`); the old `calibrated`/`stale`/`missing` artifact tally is gone with the table it read.
...
**Hot-pixel correction** (`calibration_library::cosmetic`): map computed once per distinct resolved master dark — hot = `value > median + HOT_SIGMA·1.4826·MAD`, `HOT_SIGMA = 10.0` (the external reference's high-sigma default); zero MAD or over a 5% safety-cap flags → no map, correction honestly skipped. Replacement is a neighbourhood median: mono → 3×3 window; CFA → stride-2 same-channel cells (the 5×5 window's same-phase pixels), before debayering. Stamps `ATH_CHPX` (replaced-pixel count) on every corrected output.
