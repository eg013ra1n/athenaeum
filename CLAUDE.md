...
- **Deletion is always allowed, always honest** (§2 matrix, `library/delete.rs`): per file pending-remove → audit row → unlink → `seen.mark_deleted`; one file's failure never stops the pass. The audit lands BEFORE the unlink (retention's own contract), so a failed unlink can leave a row for a file still on disk. **Forget seam (T9b)**: the watcher's emitted-paths set short-circuits *before* the seen store, so every in-app deletion (Library *and* retention) broadcasts one batched `WatcherForget::forget` — a re-created file re-enqueues within the run; a file deleted OUTSIDE Perseus is re-detected only after an agent restart (deliberate — auto-forget on a stat flap would re-send a night off a blinking share).
