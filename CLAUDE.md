...
- **Duplicate detection**: three keys, all XXH3_64, deliberately not interchangeable (spec `docs/superpowers/specs/2026-08-27-duplicate-detection-design.md` §2.5). Default: raw sub-frames by `fits_header.header_fingerprint` (+ size + filename, zero I/O), masters/processed by `files.strong_hash` (full file, header-shortlisted). Opt-in `duplicates.use_content_hash`: everything by `files.content_hash` (3 × 512 KB sampling). `content_hash` has ONE bulk producer — the content-index job (`api::content_index`, autostarts after a scan when sync is configured or content grouping is on; never the scan itself); `strong_hash` is banked by every full read (master-hash pass, deep verify, sync manifest/confirm/ingest) via `db::bank_strong_hash` under the `disk_matches_row` staleness contract. One full-hash function: `package::xxh3_full_file`. **The master-hash pass is the expensive half of scan Phase 4** (measured 2026-09-17: the two cache rebuilds + the O(folders²) folder pass total ~3.6 s on a 40k-file catalog, while the pass reads every shortlisted master in full at the volume's speed — 11.7 MB/s on the owner's SMB share), so it reports as its own scan phase `"hashing"` (per file, with the path; UI label "Verifying master duplicates") and `"caching"` names only the rebuild. Phase 4 never runs on a scan that found nothing new or modified (`new_files.is_empty()` returns first). The content index yields to any active scan (`run_content_index_yielding` parks in 500 ms steps while `ServiceContext::active_scans` is non-empty) — its per-chunk nap does not protect a scan walking the same volume. `docs/backlog-v0.5.6.md` item 9 has the numbers.
