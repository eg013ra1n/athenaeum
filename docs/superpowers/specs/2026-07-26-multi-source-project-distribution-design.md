...
After a successful ingest of a **published** package (and after own publish, and at the coordinator's approval of a copy that landed while pending — F2), the device imports a reconstructed seed dir (`collab_seed/<package_id>`, hardlinks to the landed files) as a collection tagged `project/<project_id>/<package_id>` with `ImportMode::TryReference`:
