...
| D-12 | **Contribution approval = per-project toggle** (`require_approval`): announcements gain a `pending → published\|rejected` state machine; pending push-seeds to the coordinator (who must hold `send+receive`), review happens **in-app on the data** (Moderation tab), decision posts to hub; portal shows a read-only pending counter. Members never see/serve pending packages | accepted (owner, 2026-07-12 — collab design §2/§7, mockup round) |
| D-13 | **Permissions v1 = coordinator-only** — one admin role, full matrix in collab design §5a (admin on portal, data in app; approval is the one deliberate exception); delegated roles (moderator) = Phase III backlog | accepted (owner, 2026-07-12) |
