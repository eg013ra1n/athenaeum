...
**Dictionary extension (WCS card writer, `fits_writer::wcs`, 2026-09-09):** `table` (string; which SIP table — A/B/AP/BP), `i`/`j` (usize; the term's powers of u and v) and `value` (f64) on the `"SIP low-order term written; CD fold not applied"` warn. `len` (usize; a buffer's element count — the `data.len()` the `integrate_probe` example's measurement path can hand in when a plane's length disagrees with its own `width`/`height`) on `stacking::psf_signal`'s `"background residual: plane length does not match the geometry"` warn, alongside the already-canonical `width` / `height`.
