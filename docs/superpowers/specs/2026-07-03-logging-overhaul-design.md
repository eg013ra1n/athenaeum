...
**Dictionary extension (master-build lifecycle logging, `api::masters`, integration-throughput cycle Task 7, 2026-09-06):** `imagetyp` (string; the calibration set's frame type — `"Dark"`/`"Flat"`/`"Bias"`/`"DarkFlat"` — on the `"master build started"` info; already used informally, uncatalogued, by the scanner's calibration-integration debugs (`calibration::scan_integration`), formalized here); `recipe` (string; the resolved `IntegrationRecipe::describe()`, e.g. "Average \| Winsorized sigma (3.0/3.0)", on that same info); `budget_mb` (u64; `io.band_budget_bytes` converted to MB, the band-size ceiling the run resolved to); `read_threads` (usize; `io.read_concurrency` — read concurrency is `image_pool`'s size, i.e. a CPU-derived number applied to an I/O problem (spec §8), logged so a pending SSD/NAS measurement can be read against the policy that actually produced it); `storage` (string; `StorageClass::as_str()` — `"local"`/`"network"` — the storage class `io_policy::resolve` classified the set's paths as, logged alongside `budget_mb`/`read_threads` on that same info for the same reason). Reuses `duration_ms` (elapsed wall time from budget resolution to the engine returning — NOT the whole build; write + register happen after this line) on the `"master build finished"` info, alongside `read_ms`/`combine_ms` (u64; `IntegrationOutput::read_duration`/`combine_duration` in ms — the engine's own phase split, since only it can tell the two bottlenecks apart), `read_mb_s` (u64; `bytes_read` divided by `read_duration`'s seconds, rounded, `0` when the duration is `0`), and `band_rows`/`bands` (usize; the two numbers the band budget actually decided — for a FLAT these describe pass 2 only, see the `IntegrationOutput` field docs and `EngineProgress::on_band`'s doc for why that disagrees with `bytes_read`/`read_duration`, which span both passes).
