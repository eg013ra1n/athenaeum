...
1. **`Offer { package_id, entries: Vec<OfferEntry> }`** (provider → receiver), `OfferEntry { rel_path: String, sampling_hash: String, byte_size: u64 }`.
   `sampling_hash` = `duplicates::compute_xxhash` (first/middle/last 512 KB) — the same value the receiver stores in `files.content_hash` (filled by the content-index job, `api::content_index`; never by a scan). Sender source of the value: both the **app** (`engine::build_offer`) and **Perseus** compute it on the staged payload file with `compute_xxhash` — the sender's own `files.content_hash` is not consulted, so the index matters only on the receiving side. *(Corrected 2026-08-28; the original text said the app reads it from its DB.)*
