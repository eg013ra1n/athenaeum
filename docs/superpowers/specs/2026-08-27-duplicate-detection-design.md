...
| D3c | *(added 2026-08-28, hash cleanup)* `strong_hash` is also banked by every other full read of a file — sync's manifest build, dedup confirm and ingest, and deep verify — through one helper, `db::bank_strong_hash`, under the same `disk_matches_row` staleness contract | The bytes were read anyway; a current row banks the digest, a stale row is never written. The dedup confirm decides from a banked hash without reading, so a re-offer of confirmed files costs zero I/O. `sync_receipts.xxh3` stays the ingest ledger; `strong_hash` is a cache. |
