...
| D2 | `metadata_hash` stays as a column, stops being a duplicate key | It is still written on insert and read by `MissingMetadataRow`'s `has_duplicate` flag. Removing the column is a separate, larger change with no benefit here. |
