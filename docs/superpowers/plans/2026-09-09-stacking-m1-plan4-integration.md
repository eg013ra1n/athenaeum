...
- [ ] **Step 5: Write the note** (tables: external baselines; our per-plane stats; ratios; pixel/star-level comparison; timing; verdict per target; findings and rulings; carry-forwards for Plan 5 and M2) and commit: `docs(stacking): checkpoint B — weighted integration against the external masters`.
