...
- **R-M4b-1 Scale per frame, two sources, one field.** `GroupFrame.pixel_scale_arcsec: Option<f64>` and `GroupFrame.scale_source: ScaleSource { Solve, Header }` (`None` when neither exists). Solve wins over header (a solve measures, a header assumes). Header formula: `206.2648 · xpixsz_um · xbinning / focallen_mm`, `None` when any factor is missing, non-finite or ≤ 0. `XPIXSZ` is the physical pixel size BEFORE binning (the FITS convention), hence the `xbinning` factor.
...
- **R-M4b-9 Acceptance data.** LDN 1272 has no ×2 group; Task 6 builds one: 24 mono lights software-binned 2×2 (mean, `XBINNING = YBINNING = 2`, `NAXIS1/2` halved, `XPIXSZ` kept, `OBJECT`/`DATE-OBS`/coordinates kept, filenames `bin2_<stem>.fits`) plus the matching master dark and flat binned the same way and imported into the Calibration Library as foreign masters (the scanner's `is_master` path), scanned into the catalog (they cluster into set 109 by coordinates), plate-solved through the app's own queue (so WCS seeds exist), then two runs — co-registered and native. If the owner points at a real mixed-scale set before Task 6 runs, that set replaces the synthetic group; the synthetic recipe stays in the note either way.
...
### Task 6 (controller-run): M4b acceptance run on LDN 1272 + a synthetic bin-2 group

**Setup (ruling R-M4b-9):** a script in the session scratchpad bins 24 mono lights 2×2 (mean; `XBINNING = YBINNING = 2`, `NAXIS1 = 3112`, `NAXIS2 = 2084`, other cards copied) to `<scan root>/LDN1272-bin2/lights/bin2_<stem>.fits`, and the mono master dark + master flat the same way to `<Calibration Library root>/…/` as `master_dark_*_bin2_*.fits` / `master_flat_*_bin2_*.fits` (the library's own naming, `ATH_FNRM` recomputed by the flat's central-third mean); scan; confirm the bin-2 lights joined set 109 and the matcher linked the bin-2 masters (Coverage tab); plate-solve the 24 frames (Analysis → solve queue); then run A (co-registered, LN on, drizzle 2× on) and run B (native, same).

**Measurements (note `docs/superpowers/research/<date>-m4b-acceptance-run.md`):**

| Metric | Target | How |
| ---- | ---- | ---- |
| Plan warnings | exactly one `×2.0` warning naming `mono__NoFilter__bin2__180s`; no blocker | plan JSON |
| Groups table | the bin-2 group shows `1.55"` (solve) and the `×2.0` badge | click-through |
| Registration of the bin-2 group (run A) | ≥ 23/24 registered, `scale` 2.00 ± 1 %, model suffix `+wcs` on every registered frame, RMS ≤ 1.0 reference px | `stacking_run_frames` |
| Run A master geometry | bin-2 master `6224×4168` (the reference's), level within 1 % of the bin-1 master's over the common field, star centroids of 50 matched stars within 0.5 px of the bin-1 master's | `fitsdiff.py`-style medians, `samestar.py` |
| Run A drizzle of the bin-2 group | level within 1 %, coverage ≥ 0.999 | results card |
| Run B (native) | bin-2 master `3112×2084`, `ATH_RGEO = 'native'`, its own WCS (`CDELT`-equivalent ≈ 1.55"/px), `reference #<bin-2 frame>` on the results card; the bin-1 and OSC masters unchanged vs run A within noise | headers + results |
| Same-scale groups (mono bin-1, OSC) in both runs | registration records identical to the M4a acceptance's (no `+wcs`, same inlier counts ± 2 %) | `stacking_run_frames` |
| Time | the bin-2 group's register stage ≤ 30 s per pass | `stages[register]` |

**Docs:** the acceptance note; `docs/superpowers/open-items.md` (the owed real-set smoke by the owner); `CLAUDE.md` Stacking (the M4b acceptance sentence); spec §15 closed; memory.
