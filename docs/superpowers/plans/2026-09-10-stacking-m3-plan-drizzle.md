...
- **R-M3-7 Memory refusal, not swapping.** Before allocating, the stage computes `need = (channels + 2)·out_w·out_h·4 + w·h·4 + (2·w·h·4 if LN)` bytes; when `need > total_ram_bytes()/2` (the fan-out's existing probe; unknown total → refuse above 4 GB) the GROUP's drizzle fails with `"drizzle {s}x needs ≈ {need} but only {have} is available; use a smaller scale"` — the master is already written, the group row keeps its `master_path`, `drizzle_path` stays `NULL`, the run's warnings carry the message, the run itself is not failed.
...
- Produces (`wcs.rs`):
  ...
  /// `CRPIX' = s·CRPIX − (s − 1)/2` (FITS 1-based centres; a reference pixel's centre lands
  /// between its s×s output pixels), `CD' = CD / s` (every element), `pixel_scale' = pixel_scale / s`,
