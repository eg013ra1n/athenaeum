...
- [ ] **Step 5: variants.** (UPDATED 2026-07-26: rustafits' JPEG encoder is pure Rust now — turbojpeg was removed in favor of `crates/jpeg-encode`; do NOT add cmake/nasm anywhere.) `build_arm64.sh`: change the deb build from `--no-default-features` to `--no-default-features --features preview` so tray stays off but preview stays on. Confirm `cargo build -p perseus --no-default-features` still compiles (stub route).
