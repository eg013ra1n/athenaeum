...
- **Do NOT touch `account/client.rs::set_role` or the `DeviceRole` enum** — Perseus (`perseus/src/account.rs`) still calls `set_role(.., DeviceRole::Capture, ..)` and uses `DeviceRole` in `auto_pick_primary`. Deleting either now breaks `cargo build --workspace`. **`HubClient::set_role` + `DeviceRole` are deleted in Task 6** when Perseus is rewritten off them. After Task 2, `set_machine_role` (the only *core* caller of `set_role`) is gone, so `set_role` is exercised only by Perseus + its own tests — no dead-code warning in core.
...
**Deferred deletions this task also performs (app-only symbols kept compiling through Task 2):** delete `resolve_capture_peer`, `persist_peer_resolution`, `peer_from_resolution` (`api/sync.rs`), `account_pairing` (`api/account.rs`), and the `ACCOUNT_ROLE`/`ACCOUNT_PEER_DEVICE_ID` consts (`settings/mod.rs`) plus their now-orphaned clears in `clear_local_session`/`sign_in_verify`. Keep `peer_addr_with_relays` + relay helpers (the new `ensure_sender_engine` dials with them).

**Still NOT removed here — Perseus consumes them, deletion is Task 6:** the `DeviceRole` enum, `HubClient::set_role`, and any `sync/pairing.rs` helpers Perseus still uses (`AccountPairing`/`PeerResolution`/`resolve_peer`/`fetch_primary_node_id`). Removing them now breaks `cargo build --workspace` (Perseus). Task 3's core send path must not depend on them; leave them alive for Perseus. After Task 6 rewrites Perseus, Task 6 deletes them and core is fully role-free. Regenerate TS if any exported type changed; gate on `cargo build --workspace` + `cargo test -p athenaeum-core` (Perseus still green because its role/pairing deps remain).
...
- [ ] **Step 3: Implement** — `compute_rel_path` (strip_prefix, `to_slash`, label when `capture_dirs_resolved().len() > 1` using the same sanitizer discipline as core's slug — lowercase `[a-z0-9._-]`); thread the owning `capture_dir` from `watcher.rs` → the enqueue consumer → `build_package_for_file`; `verify_and_register` passes `DeviceCapability::Perseus` to `verify` and **removes** the `set_role`/`auto_pick_primary`/primary-pairing logic; `device_name()` → `config.device_name.clone().unwrap_or_else(|| default_device_name("perseus", &node_id_hex))`; add `targets`/`device_name` to `Config` + template + `validate_ready` (require ≥1 target instead of a primary).

  **Cross-crate cleanup (now that Perseus no longer uses them):** delete `HubClient::set_role` + its tests (`athenaeum-core/src/account/client.rs`), the `DeviceRole` enum (`account/mod.rs`), and any `sync/pairing.rs` primary-resolution helpers left dead once Perseus stops calling them (`fetch_primary_node_id`/`resolve_peer`/`AccountPairing`/`PeerResolution` — remove those with **no** remaining caller; keep `peer_addr_with_relays`). Also drop Perseus's `PairingCache` primary fields / `auto_pick_primary` / `resolve_pairing` single-primary path (replaced by target resolution — but the actual multi-engine wiring is Task 7; here just make registration + config compile with a single first-target fallback if needed). **Gate: `cargo build --workspace` + `cargo test -p athenaeum-core` + `cargo test -p perseus` all green, warning-free** — this is the task where core becomes fully role-free.
