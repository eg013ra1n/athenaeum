...
- **Upload speed limit.** Settings → Sync gains "Upload speed limit (MB/s)": one number that caps this device's total sync upload bandwidth so a big transfer never chokes your connection (0 or empty = unlimited, the default). On a Perseus observatory agent set `max_upload_mbps` in perseus.toml or the web Settings tab — it applies live, and a cap of e.g. 8 MB/s keeps SSH responsive while a night's worth of frames uploads. Applies to uploads only; each device's downloads are bounded by the sending device's limit.
