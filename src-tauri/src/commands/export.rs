...
pub async fn export_frame_set(
    ...
    let _ = app_handle.emit("export-progress", ExportProgress {
        ...
        progress: 0.0,
        message: "Collecting frame data...".to_string(),
        current_file: None,
    });
    ...
    let _ = app_handle.emit("export-progress", ExportProgress {
        stage: ExportStage::Collecting,
        progress: 100.0,
        message: format!("Found {} light frames", export_data.total_light_frames),
        current_file: None,
    });
    ...
    if matches!(
        ...
        // Emit organizing start
        let _ = app_handle.emit("export-progress", ExportProgress {
            stage: ExportStage::Organizing,
            progress: 0.0,
            message: "Organizing files...".to_string(),
            current_file: None,
        });
    ...
    if matches!(
        ...
        match generate_scripts(&config, &export_data) {
            Ok(scripts) => {
                ...
                // Emit script generation complete
                let _ = app_handle.emit("export-progress", ExportProgress {
                    stage: ExportStage::GeneratingScripts,
                    progress: 100.0,
    ...
    let _ = app_handle.emit("export-progress", ExportProgress {
        ...
        progress: 100.0,
        message: format!("Export complete! {} files, {} scripts", result.files_organized, result.scripts_generated.len()),
        current_file: None,
    });
...
/// Export a frame set using the new v2 workflow with ExportGroups
///
/// This version uses:
/// - ExportGroups (grouped by filter + camera type)
/// - Calibration subgroups (different calibration set combinations)
/// - MasterCreationPlan (topologically sorted master creation)
/// - Separate scripts for masters and each group
#[tauri::command]
pub async fn export_frame_set_v2(
    state: State<'_, AppState>,
    app_handle: tauri::AppHandle,
    frame_set_id: i64,
    output_dir: String,
    ...
    // Build config - workflow is now auto-determined per group
    let config = ExportConfig {
        frame_set_id,
        output_dir: PathBuf::from(&output_dir),
        target: ExportTarget::default(),
    ...
    let _ = app_handle.emit(
        ...
        ExportProgress {
            ...
            progress: 100.0,
            message: format!(
                "Found {} light frames in {} groups",
                export_data.total_light_frames,
                export_data.groups.len()
            ),
            current_file: None,
        },
    );
    ...
    // Step 2: Generate scripts using v2 (if needed)
    if matches!(
        export_mode,
        ExportMode::GenerateScripts | ExportMode::OrganizeAndScript | ExportMode::DirectExecution
    ) {
        let _ = app_handle.emit(
            ...
            ExportProgress {
                stage: ExportStage::GeneratingScripts,
                progress: 0.0,
                message: "Generating Siril scripts (v2)...".to_string(),
                current_file: None,
            },
        );

        match generate_scripts_v2(&config, &export_data) {
            Ok(scripts) => {
                result.scripts_generated = scripts
                    .iter()
                    .map(|p| p.to_string_lossy().to_string())
    ...
    if export_mode == ExportMode::DirectExecution {
        ...
        // Run scripts in order (master script first, then group scripts)
        for script_path in &result.scripts_generated {
            if let Err(e) =
