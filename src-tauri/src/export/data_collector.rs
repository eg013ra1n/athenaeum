...
//!
//! V3: Builds calibration branches for nested folder hierarchy

use crate::export::models::{
    CalibrationBranch, CalibrationSetInfo, CalibrationSubgroup, CalibrationSummary, CameraType,
    ExportCalibrationSet, ExportData, ExportDataV3, ExportFrame, ExportGroup, FilterExportGroup,
    MasterCreationPlan, MasterInfo, sanitize_folder_name,
};
...
// ============================================================================
// V3: Calibration Branch Building for Nested Folder Hierarchy
// ============================================================================

/// Collect export data using V3 nested hierarchy structure
///
/// Groups frames by camera, calibration chain, AND filter to create branches.
/// Each branch represents a unique path: Camera → Bias → Dark → Flat → Filter → Lights
pub fn collect_export_data_v3(conn: &Connection, frame_set_id: i64) -> Result<ExportDataV3> {
    println!("📦 Collecting V3 export data for frame set {}", frame_set_id);

    // Get frame set info
    let (frame_set_name, object_name) = get_frame_set_info(conn, frame_set_id)?;
    println!("  Frame set: {}, object: {:?}", frame_set_name, object_name);

    // Get all light frames
    let light_frames = get_light_frames_for_frame_set(conn, frame_set_id)?;
    println!("  Found {} light frames", light_frames.len());

    // Build calibration branches (grouped by camera + calibration chain + filter)
    let branches = build_calibration_branches(conn, &light_frames)?;
    println!("  Built {} calibration branches", branches.len());

    // Build master creation plan from branches
    let master_plan = build_master_plan_from_branches(conn, &branches)?;
    println!("  {} masters to create", master_plan.masters.len());

    // Collect unique cameras and filters
    let cameras: Vec<String> = branches
        .iter()
    ...
    let filters: Vec<String> = branches
        .iter()
    ...
    // Calculate totals
    let total_light_frames: i32 = branches.iter().map(|b| b.light_frames.len() as i32).sum();
    let total_exposure_seconds: f64 = branches.iter().map(|b| b.total_exposure()).sum();
...
/// Build calibration branches from light frames
///
/// Groups frames by:
/// 1. Camera name (instrume)
/// 2. Calibration chain (bias_id, dark_id, flat_id, darkflat_id)
/// 3. Filter name
///
/// Each unique combination becomes a separate branch.
fn build_calibration_branches(
    conn: &Connection,
    light_frames: &[ExportFrame],
) -> Result<Vec<CalibrationBranch>> {
    println!("  🌿 Building calibration branches");

    // Branch key: (camera, bias_id, dark_id, flat_id, darkflat_id, filter)
    type BranchKey = (String, i64, i64, i64, i64, Option<String>);
    let mut branch_groups: HashMap<BranchKey, Vec<ExportFrame>> = HashMap::new();
    let mut branch_calibrations: HashMap<BranchKey, FullCalibrationChain> = HashMap::new();

    // Group frames by their complete calibration chain + filter
    for frame in light_frames {
        let camera = frame.instrume.clone().unwrap_or_else(|| "unknown".to_string());
        let filter = frame.filter.clone();

        // Get full calibration chain for this frame
        let chain = get_full_calibration_chain(conn, frame.frame_id)?;
        ...
        branch_groups.entry(key.clone()).or_default().push(frame.clone());
        branch_calibrations.entry(key).or_insert(chain);
    }

    println!("    Found {} unique branches", branch_groups.len());

    // Build CalibrationBranch for each group
    let mut branches = Vec::new();

    for (key, frames) in branch_groups {
        let (camera_name, bias_id, dark_id, flat_id, darkflat_id, filter) = key;
        let camera_folder_name = sanitize_folder_name(&camera_name);

        let chain = branch_calibrations.get(&(
            camera_name.clone(),
            bias_id,
            dark_id,
            flat_id,
        ...
        let branch_id = CalibrationBranch::make_branch_id(
            &camera_name,
            bias_id,
            dark_id,
            flat_id,
        ...
        // Build CalibrationSetInfo for each level
        let bias_info = if bias_id > 0 {
            chain.and_then(|c| c.bias_info.clone())
        } else {
            None
        };

        let dark_info = if dark_id > 0 {
            chain.and_then(|c| c.dark_info.clone())
        } else {
            None
        };

        let flat_info = if flat_id > 0 {
            chain.and_then(|c| c.flat_info.clone())
        } else {
            None
        };

        let darkflat_info = if darkflat_id > 0 {
            chain.and_then(|c| c.darkflat_info.clone())
        } else {
            None
        };

        branches.push(CalibrationBranch {
            branch_id,
            camera_name,
            camera_folder_name,
            bias_id,
    ...
    // Sort branches by branch_id for consistent ordering
    branches.sort_by(|a, b| a.branch_id.cmp(&b.branch_id));

    Ok(branches)
}

/// Full calibration chain for a frame
#[derive(Debug, Clone)]
struct FullCalibrationChain {
    bias_id: i64,
    dark_id: i64,
    flat_id: i64,
    darkflat_id: i64,
    bias_info: Option<CalibrationSetInfo>,
    dark_info: Option<CalibrationSetInfo>,
    flat_info: Option<CalibrationSetInfo>,
    darkflat_info: Option<CalibrationSetInfo>,
}

/// Get the complete calibration chain for a frame
fn get_full_calibration_chain(conn: &Connection, frame_id: i64) -> Result<FullCalibrationChain> {
    // Get direct calibration links
    let mut flat_id = 0i64;
    let mut dark_id = 0i64;
    let mut bias_id = 0i64;
    let mut darkflat_id = 0i64;

    let mut flat_info: Option<CalibrationSetInfo> = None;
    let mut dark_info: Option<CalibrationSetInfo> = None;
    let mut bias_info: Option<CalibrationSetInfo> = None;
    let mut darkflat_info: Option<CalibrationSetInfo> = None;

    // Get frame's direct calibration links
    let mut stmt = conn.prepare(
        "SELECT calibration_set_id, calibration_type, match_score, date_warning, temp_warning
         FROM calibration_set_to_frames
         WHERE source_id = ?1 AND source_type = 'frame'",
    ...
    for (set_id, cal_type, match_score, date_warn, temp_warn) in links {
        match cal_type.as_str() {
            "Flat" => {
                flat_id = set_id;
                flat_info = Some(build_calibration_set_info(conn, set_id, match_score, date_warn, temp_warn)?);

                // Get darkflat from flat's sub-calibrations
                if let Some(ref fi) = flat_info {
                    if let Some(ref df) = fi.dark_flat {
                        darkflat_id = df.set_id;
                        darkflat_info = Some((**df).clone());
                    }
                }
            }
            "Dark" => {
                dark_id = set_id;
                dark_info = Some(build_calibration_set_info(conn, set_id, match_score, date_warn, temp_warn)?);
            }
            "Bias" => {
                bias_id = set_id;
                bias_info = Some(build_calibration_set_info(conn, set_id, match_score, date_warn, temp_warn)?);
            }
            _ => {}
        }
    }

    // If bias not directly linked, get it from dark's sub-calibration
    if bias_id == 0 {
        if let Some(ref di) = dark_info {
            if let Some(ref b) = di.bias {
                bias_id = b.set_id;
                bias_info = Some((**b).clone());
            }
        }
    }

    // If still no bias, check flat's dark's bias
    if bias_id == 0 {
        if let Some(ref fi) = flat_info {
            if let Some(ref d) = fi.dark {
                if let Some(ref b) = d.bias {
                    bias_id = b.set_id;
                    bias_info = Some((**b).clone());
                }
            }
...
/// Build master creation plan from calibration branches
fn build_master_plan_from_branches(
    conn: &Connection,
    branches: &[CalibrationBranch],
) -> Result<MasterCreationPlan> {
    // Collect all unique calibration set IDs with their types
    let mut all_sets: HashMap<i64, String> = HashMap::new();
    let mut dependencies: HashMap<i64, Vec<i64>> = HashMap::new();

    for branch in branches {
        // Collect from each calibration level
        ...
        if let Some(ref darkflat) = branch.darkflat_info {
            collect_calibration_sets_recursive(darkflat, &mut all_sets, &mut dependencies);
        }
    }
