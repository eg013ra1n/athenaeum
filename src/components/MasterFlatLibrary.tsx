...
export default function MasterFlatLibrary({ instrume, onClose, isTabView = false }: MasterFlatLibraryProps) {
  ...
  const checkAndLoadLibrary = async () => {
    try {
      setLoading(true);
      setError(null);
      const hasLibrary = await api.invoke<boolean>("has_master_flat_library", { instrume });

      if (hasLibrary) {
        const result = await api.invoke<CalibrationSetDetail[]>("get_master_flat_library", { instrume });
        setSets(result);
