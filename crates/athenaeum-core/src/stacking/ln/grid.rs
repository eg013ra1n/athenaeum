...
mod tests {
    ...
    /// Cross-scale pin (M4b Task 4, ruling R-M4b-6): the engine's per-pixel
    /// LN lookup (mirrored exactly by the drizzle stage driver's own
    /// `deposit_band`, `stacking::drizzle::mod`, and by
    /// `integration::engine`'s band loop for the integrate stage) forward-
    /// maps a source pixel through the frame's `PixelMap` into REFERENCE
    /// coordinates, then rounds (`floor(v + 0.5)`, `drizzle::geom::map_drop`'s
    /// own documented convention) to index the grid — the grid is built,
    /// and only ever evaluated, in reference geometry and never sees a raw
    /// source coordinate. This pins that a ×2 registration scale still
    /// reads the REFERENCE cell, mirroring `drizzle::mod::tests::
    /// local_normalization_index_is_not_transposed` (which pins the same
    /// convention at an identity map).
    #[test]
    fn a_grid_lookup_through_a_double_scale_map_reads_the_reference_cell() {
        use crate::geometry::{Linear, LinearKind, PixelMap};

        const REF_W: usize = 400;
        const REF_H: usize = 300;

        // `b` ramps with the reference NODE column (stride 32 at scale
        // 256), 10.0 per node, so reading the wrong (un-mapped) pixel
        // column disagrees by a known, unmissable amount — the evaluated
        // surface reproduces a ramp in the node values exactly (see this
        // module's own doc), so `b` at pixel x is `(x / stride) * 10.0`.
        let mut grid = LnGrid::constant(REF_W, REF_H, 256, 1.0, 0.0);
        let gw = grid.gw;
        for j in 0..grid.gh {
            for i in 0..gw {
                grid.b[j * gw + i] = i as f32 * 10.0;
            }
        }

        // Subject (200×150) → reference (400×300) at a pure ×2
        // registration scale — the same map the resampler and drizzle pins
        // use (`integration::registered_source::tests`,
        // `drizzle::mod::tests`).
        let fwd = Linear {
        ...
        let (sub_x, sub_y) = (100.0f64, 75.0f64);
        let (u, v) = map.forward(sub_x, sub_y);
        assert_eq!(
            (u, v),
            (200.0, 150.0),
            "sanity: the ×2 map's own forward mapping"
        );

        // The band loop's own rounding convention: `floor(v + 0.5)`.
        let ref_x = (u + 0.5).floor() as usize;
        let ref_y = (v + 0.5).floor() as usize;

        let mut a_row = vec![0f32; REF_W];
        let mut b_row = vec![0f32; REF_W];
        let mut scratch = LnScratch::for_grid(&grid);
        grid.evaluate_row_into(ref_y, &mut a_row, &mut b_row, &mut scratch);

        // The one assertion: the mapped REFERENCE column (200) reads a
        // value the raw, un-mapped SOURCE column (100) does not — a lookup
        // that mistakenly indexed the grid by the source pixel instead of
        // the forward-mapped reference coordinate would read `b_row[100]`
        // here, off by ≈31.25 for this ramp (column 200 ≈ 62.5, column 100
        // ≈ 31.25 — see the ramp's own doc above), not the sub-1-unit
        // smoothing bias the B-spline reconstruction itself carries between
        // neighbouring pixels.
        assert!(
            (b_row[ref_x] - b_row[sub_x as usize]).abs() > 20.0,
            "reference-column read {} at x={ref_x} vs a source-column read {} at x={}: \
             the lookup must use the mapped reference coordinate, never the raw source one",
            b_row[ref_x],
            b_row[sub_x as usize],
            sub_x as usize
        );
    }
}
