...
//! actual queue-admission + integration + write + register — mirroring
//! `api::analysis::analyze_frame_set`'s "sync core fn + `&dyn
//! ProgressEmitter`, wrapper owns the blocking boundary" shape, except here
//! the wrapper never blocks at all: it just spawns and returns.
...
pub fn preview_master_build(
    ...
    let (flat_precal, warnings) = if is_flat {
        let (_precal, desc, warnings) = resolve_flat_precal(&conn, set_id, set.exptime, recipe.synthetic_bias)?;
        (desc, warnings)
    } else {
        (None, Vec::new())
    };
...
fn run_build(
    ...
    let is_flat = set.imagetyp == "Flat";
    let resolved_combine = resolve_combine(recipe.combine, &set.imagetyp, set.frame_count);
    let (precal, precal_desc, _warnings) = if is_flat {
        resolve_flat_precal(&conn, set_id, set.exptime, recipe.synthetic_bias)?
    } else {
        (FlatPrecal::None, None, Vec::new())
    };

    // Release the pooled connection before the (potentially long) banded
    // integration pass below — it does no DB work, and the pool only has a
    // handful of slots shared with every other in-flight request.
    drop(conn);
