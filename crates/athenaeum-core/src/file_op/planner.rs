...
/// Build and persist a Delete plan.
///
/// `targets`: absolute paths the user wants to delete (mix of files and
/// directories). For directories, all contained files are added to the plan.
/// Empty directories left after the file deletions are removed in the executor.
///
/// Permanent delete: no undo, no trash. The UI must have already confirmed.
pub fn build_delete_plan(conn: &Connection, targets: Vec<PathBuf>) -> Result<FileOpPlan> {
    let scan_roots = get_scan_roots(conn).context("listing scan roots")?;
    let scan_root_paths: Vec<PathBuf> = scan_roots
        .iter()
        .filter(|r| r.enabled)
    ...
    for t in &targets {
        if !is_inside_any(t, &scan_root_paths) {
            return Err(anyhow!(
                "target '{}' is not inside any scan root",
                t.display()
            ));
        }
        if !t.exists() {
            return Err(anyhow!("target '{}' does not exist", t.display()));
        }
    }

    // Collect file paths AND every subdirectory we walk past — files are
    // deleted first, then dirs are removed deepest-first so the whole
    // hierarchy gets cleared (not just the leaves).
    ...
    for tgt in &targets {
        if tgt.is_file() {
            if seen_files.insert(tgt.clone()) {
                let size = fs::metadata(tgt)?.len();
                file_paths.push((tgt.clone(), size));
            }
        } else if tgt.is_dir() {
            for entry in WalkDir::new(tgt).follow_links(false) {
                let entry = entry?;
                ...
                // Symlinks are intentionally skipped — neither deleted nor
                // followed, mirroring the move planner.
            }
        } else {
            return Err(anyhow!(
    ...
    if file_paths.is_empty() {
        // All targets were empty directories — still allow the operation; the
        // executor's directory-cleanup pass will remove them.
    }

    let total_bytes: i64 = file_paths.iter().map(|(_, s)| *s as i64).sum();
    let total_files = file_paths.len() as i64;
    let source_root = common_root(&targets)
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_default();
    ...
    // Also add a planned row for every directory we walked through, plus any
    // top-level target dirs that weren't discovered during the walk (i.e.
    // empty top-level selections). The executor sorts these deepest-first
    // and removes them after files are gone, clearing the whole hierarchy.
    for tgt in &targets {
        if tgt.is_dir() && seen_dirs.insert(tgt.clone()) {
            dir_paths.push(tgt.clone());
        }
    }
    for d in &dir_paths {
        fdb::insert_operation_file(
