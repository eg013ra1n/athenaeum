...
docker run --rm --platform linux/arm64 \
  ...
  # now that rustafits encodes JPEG via libjpeg-turbo-rs instead of the C
  # libjpeg-turbo (the old jpeg-encode helper crate is gone; athenaeum-core
  # carries no JPEG encoder outside the render feature). While turbojpeg-sys
  # was still in Cargo.lock this container would have needed cmake + nasm.
